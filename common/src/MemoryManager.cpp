...
char  LOGFILE[40]           = "EngineMemoryLogFile.log"; 
...
*/
COMMON_API void *AllocateMemory( const char *file, int line, size_t size, ALLOC_TYPE type, void *address ) 
{
	MemoryNode *memory;
	...
	// stl类型的内存分配，不输出。
	if (s_manager->m_logAlways && (line!=0)) {
...
bool  MemoryManager::validateMemoryUnit( MemoryNode *node )
{
	...
	// STL 分配的内存不输出。
	if (m_logAlways && totalBytesUsed != node->reportedSize && (node->sourceLine>0)) 
	{          // Report the percentage of waisted memory space.
		this->log( "Unused Memory Detected   : %-40s %8s(0x%08p) : %s",     
