 ...
 */
//------------------------------------------------------------------------------------------------
//! ATTENTION : 这个宏设置为1，则全局的重栽new,delete,跟踪内存分配。如果设为零，则使用windows的
//!             new和delete.
...
COMMON_API void *AllocateMemory( const char *file, int line, size_t size, ALLOC_TYPE type, void *address = NULL ); 
/* AllocateMemory():
...
COMMON_API void deAllocateMemory( void *address, ALLOC_TYPE type );
/* deAllocateMemory():
...
COMMON_API void setOwner( const char *file, int line );
/* setOwner():
...
class COMMON_API Initialize { public: Initialize(); };
...
// &&&&& IF these func will never be called ,River comment it off @ 5/11 2003
...
//! 重载stl中的new.此函数用于兼容stl.
inline void* operator new( size_t size ) 
{
  return AllocateMemory( "std",0, size, MM_NEW ); 
}
...
inline void* operator new[]( size_t size )
{
  return AllocateMemory( "std", 0, size, MM_NEW_ARRAY );
}
...
COMMON_API inline void operator delete( void *address, const char *file, int line )   
{ free( address ); }
COMMON_API inline void operator delete[]( void *address, const char *file, int line ) 
{ free( address ); }
