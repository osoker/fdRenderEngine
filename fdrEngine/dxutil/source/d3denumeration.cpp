...
# include "../include/D3DEnumeration.h"
...
// Desc: Returns the number of alpha channel bits in the specified D	3DFORMAT
//-----------------------------------------------------------------------------
...
CD3DEnumeration::CD3DEnumeration()
{
	...
	// River added @ 2004-3-11,可以使用mixed设备。
    AppUsesMixedVP = true;
    ...
    for( UINT iaf = 0; iaf < pAdapterFormatList->Count(); iaf++ )
    {
        ...
        for( UINT ibbf = 0; ibbf < backBufferFormatArrayCount; ibbf++ )
        {
            ...
            for( UINT iiw = 0; iiw < 2; iiw++)
            {
                ...
                pDeviceCombo->pVertexProcessingTypeList = new CArrayList( AL_VALUE, sizeof( ose_VertexProcessingType ) );
...
{
	guard;// CD3DEnumeration::BuildVertexProcessingTypeList() );

    ose_VertexProcessingType vpt;

	osassert( ConfirmDeviceCallback!=NULL );
	...
	if ((pDeviceInfo->Caps.DevCaps & D3DDEVCAPS_HWTRANSFORMANDLIGHT) != 0)
	{
		...
		{
			if ( ConfirmDeviceCallback( m_pD3D,&pDeviceInfo->Caps, PURE_HARDWARE_VP, 
		...
		if ( ConfirmDeviceCallback( m_pD3D,&pDeviceInfo->Caps, HARDWARE_VP, 
			pDeviceCombo->AdapterFormat, pDeviceCombo->BackBufferFormat))
		{
			vpt = HARDWARE_VP;
			pDeviceCombo->pVertexProcessingTypeList->Add(&vpt);
		...
		//
		// 如果可以使用mixedDevice.
		if (AppUsesMixedVP && ( ConfirmDeviceCallback( m_pD3D,&pDeviceInfo->Caps, MIXED_VP, 
			pDeviceCombo->AdapterFormat, pDeviceCombo->BackBufferFormat)))
		{
			vpt = MIXED_VP;
			pDeviceCombo->pVertexProcessingTypeList->Add(&vpt);
	...
	//
	// 使用软件渲染的设备。
    if (  ConfirmDeviceCallback( m_pD3D,&pDeviceInfo->Caps, SOFTWARE_VP, 
        pDeviceCombo->AdapterFormat, pDeviceCombo->BackBufferFormat))
    {
        vpt = SOFTWARE_VP;
        pDeviceCombo->pVertexProcessingTypeList->Add(&vpt);
